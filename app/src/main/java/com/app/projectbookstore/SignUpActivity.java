...
public class SignUpActivity extends AppCompatActivity {
    ...
    private void checkRegistryInfo()
    {
        ...
        if(userID.length() < 8)
        {
            etStudentID.setError("Student ID is too short.");
            etStudentID.requestFocus();
            return;
        }
        ...
        {
            if(!userEmail.contains("@lpubatangas.edu.ph"))
            {
                etEmail.setError("Invalid email.");
                etEmail.requestFocus();
                return;
        ...
        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (!(dataSnapshot.child("Users").child(userID).exists())){
                    ...
                    progressDialog.dismiss();
                    Toast.makeText(SignUpActivity.this, "Please try again using another Student Number.", Toast.LENGTH_SHORT).show();
                    Intent intent = new Intent(SignUpActivity.this, MainActivity.class);
                    startActivity(intent);
                }
            }
